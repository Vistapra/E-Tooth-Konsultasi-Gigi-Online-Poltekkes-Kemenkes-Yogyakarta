...
class MessagesController extends Controller
{
    ...
    public function generateAIResponse($originalMessage)
    {
        ...
        if (!$latestMessage) {
            ...
            // Tambahkan pengecekan dan log ini
            if (!is_string($originalMessage->from_id)) {
                Log::error('MessagesController: Invalid user ID', ['userId' => $originalMessage->from_id]);
                throw new \InvalidArgumentException('User ID must be a string');
            }

            $aiResponse = $this->aiService->generateResponse($originalMessage->body, $originalMessage->from_id, $context);

            Log::info('MessagesController: AI response generated', [
            ...
            if ($aiResponse) {
                ...
                $recommendedDoctor = $this->doctorRecommendationService->getRecommendedDoctor($originalMessage->from_id, $aiResponse['keywords'] ?? []);

                if ($recommendedDoctor) {
                    $aiMessage->body .= "\n\nBased on your concerns, I recommend consulting with Dr. " . $recommendedDoctor->name . ", who specializes in " . $recommendedDoctor->specialization . ".";
                    $aiMessage->save();
                }
